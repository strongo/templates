...
func (parser *Parser) handleMKP(token Token) error {
	...
	case AT:
		if next != nil {
			...
			case KEYWORD, BRACE_OPEN: //BLK
				...
				parser.ast = parser.ast.beget(BLOCK, "")
	...
	case TEXT_TAG_OPEN, HTML_TAG_OPEN:
		...
		if parser.ast.TagName != "" {
			parser.ast = parser.ast.beget(MARKUP, tagName)
		} else {
	...
	case TEXT_TAG_CLOSE, HTML_TAG_CLOSE:
		...
		tagName = strings.Replace(tagName, "</", "", -1)
		//TODO
		opener := parser.ast.closest(MARKUP, tagName)
		...
		if parser.ast.Parent != nil && parser.ast.Parent.Mode == BLOCK {
			parser.ast = parser.ast.Parent
		}
...
func (parser *Parser) handleBLK(token Token) error {
	...
	case AT:
		if next.Type != AT {
			...
			parser.ast = parser.ast.beget(MARKUP, "")
		} else {
			next.Type = CONTENT
	...
	case AT_COLON:
		parser.subParse(token, MARKUP, true)
